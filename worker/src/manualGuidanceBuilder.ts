...
const RULE_GUIDANCE: Record<string, string> = {
  ...
  'landmark-nav-missing': 'Verificar que la navegación principal use <nav> o role="navigation" con nombre accesible si hay mas de una.',
  ...
  'autocomplete-missing': 'Verificar que los campos de datos personales usen valores autocomplete específicos: name, given-name, email, tel, address-line1, postal-code, etc.',
  ...
  'label-empty-text': 'Verificar que la etiqueta asociada tenga texto visible y descriptivo que identifique el propósito del campo.',
  'required-html5-indicator': 'Verificar que campos requeridos tengan indicación visible (texto "Obligatorio", asterisco con leyenda) y que el error se anuncie correctamente.',
  ...
  'h1-in-header': 'Verificar si el h1 en el encabezado representa el contenido único de la página o solo la marca repetida. El h1 debe ser único por página.',
  'empty-list-item': 'Verificar si el li vacio tiene propósito decorativo (usar CSS) o si es un error de maquetación que debe eliminarse.',
  'table-purpose-review': 'Determinar si la tabla presenta datos relacionados. Si es de datos, debe tener th, scope y caption; si es maquetación, usar CSS o role="presentation".',
  'title-non-interactive': 'Verificar si el title aporta información única no disponible de otro modo. Si es redundante, eliminarlo; si es necesario, hacerlo visible.',
  ...
  'content-behind-dialog-accessible': 'Confirmar que cuando el diálogo esta abierto, el contenido de fondo no recibe foco ni es legible por lector de pantalla (inert o aria-hidden en el fondo).',
  'frame-tested': 'Evaluar directamente la URL del iframe o revisar su contenido en el navegador buscando violaciones de contraste, texto alternativo, formularios y navegación por teclado.',
  ...
  'image-ignored-review': 'Verificar si la imagen con aria-hidden o alt="" transmite información visual única. Si la transmite, agregar texto alternativo descriptivo.',
  'heading-markup-review': 'Verificar si el texto que parece un título usa el nivel de encabezado correcto en la jerarquía de la página. Revisar el orden h1 > h2 > h3 sin saltos.',
};
...
function buildStepsForRule(ruleId: string, category: string): string[] {
  ...
  if (ruleId.includes('contrast')) {
    return [
      ...
      'Calcular la relación de contraste con WebAIM Contrast Checker o similar.',
  ...
  if (ruleId.includes('table')) {
    return [
      ...
      'Si es tabla de maquetación: agregar role="presentation" o reemplazar por CSS.',
  ...
  if (ruleId.includes('overlay') || ruleId.includes('dialog')) {
    return [
      'Abrir el diálogo/overlay con teclado y verificar que el foco se mueva al interior.',
      'Confirmar que Escape cierra el diálogo y devuelve el foco al elemento que lo abrio.',
      'Verificar con lector de pantalla que el nombre del diálogo es anunciado al abrirse.',
      'Confirmar que el contenido de fondo no es alcanzable con Tab mientras el diálogo esta abierto.',
    ];
  }
  return base;
}
